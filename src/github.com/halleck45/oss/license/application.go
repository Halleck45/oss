...
func (app *Application) Run(command string) {
	...
	switch(command) {
		...
		// -------------------------
		case "update":
			app.Service.SpdxService.Update()
...
//				}
		...
		// -------------------------
		case "licenses":
			licenses := app.Service.SpdxService.All()
		...
		case "search":
			...
			term := os.Args[2]
			licenses := app.Service.SpdxService.SearchMatching(term)
		...
		case "add":
			...
			spdxlicense, err := app.Service.SpdxService.Get(lic)
			if(err != nil) {
				fmt.Printf("License %s is not registred in SPDX database\n", lic)

				// search license by metaphon
				license, err := app.Service.SpdxService.SearchAround(lic);
				if(err == nil) {
